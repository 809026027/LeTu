import { fetchItems, fetchIdsByType, fetchUser,ajax } from './fetch'
...
export function ACTION_DO({ commit, state }, {name,data}) {
    console.log(name+":"+data.name+"-"+data.password);
    let result = ajax('http://127.0.0.1:1337/src/json/test.json');
    console.log(result);
    return result;
}
